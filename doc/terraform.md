...
This section explains the infrastructure layer first, because everything else depends on it.
...
## `doc/source/infra/main.tf`

### Lines 1-8: Terraform and provider setup
...
This file shows a clean Terraform pattern: use data sources for cloud discovery, locals for deterministic selection, and `for_each` for a scalable multi-node layout.

## `doc/source/infra/variables.tf`
...
## `doc/source/infra/terraform.tfvars`
...
## `doc/source/infra/README.md`

- This file is the operational runbook for provisioning and connecting to the VMs.
- It explains the order: init, apply, read private IPs, add public IPs, SSH in, then prepare Docker and image preloading.
- The README intentionally focuses on the workflow, while `main.tf` and `outputs.tf` explain the actual infrastructure model.

## Key Terraform Message for Interviewers

The infrastructure is deliberately small, but the code is structured like production IaC:

- deterministic discovery instead of hardcoded cloud IDs
- separate variable and output surfaces
- explicit network creation
- scalable VM creation with `for_each`
- helper scripts that consume Terraform outputs instead of duplicating configuration
